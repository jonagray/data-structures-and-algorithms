...
const count = (target, input) => {
  ...
  let count = 0;
  input.forEach(arr => {
    arr.map(element => element === target ? count++ : element);
  });
  return count;
};
...
const totalSum = (input) => {
  ...
  let sum = 0;
  input.forEach(arr => {
    sum = sum + arr.reduce((accumulator, current) => accumulator + current);
  });
  return sum;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(characterName => characterName.gender === 'male' || characterName.gender === 'female').map(characterName => characterName.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((shorter, taller) => Number(shorter.height) < Number(taller.height) ? shorter : taller).name;
};
