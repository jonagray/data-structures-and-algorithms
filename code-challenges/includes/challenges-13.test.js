...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map(firstLetters => firstLetters.charAt(0));
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(phoneNumbers => phoneNumbers.replace(/[() -]/g, ''));
};
...
const allHappy = (arr) => {
  // Solution code here...
  let boolean = arr.map(happy => happy.includes(':)') ? true : false);
  return boolean.includes(false) ? false : true;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  return arr.every(strings => strings.includes(target));
};
