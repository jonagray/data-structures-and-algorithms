...
const validatePin = (pin) => {
  // Solution code here...
  const validatePattern = /\b\d{4}\b/g;
  return validatePattern.test(pin);
};
...
const validateEmail = (email) => {
  // Solution code here...
  const validateEmailPattern = /^[A-Za-z0-9]+\.?[A-Za-z0-9]+?@[A-Za-z0-9]+.(net|com|org)$/g;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const validatePhoneNumberPattern = /^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/g;
  return validatePhoneNumberPattern.test(phoneNumber);
};
